...
| Modes `weave` \| `baseline` (baseline = no Weave) | Harness `Variant` is `charter` \| `baseline`, where baseline = a generic one-line prompt (no charter at all). | `weave` = full `CLAUDE.md`; `baseline` = the **same charter with ZPR5 stripped** (`stripZPR5()`), isolating Weave's marginal value instead of re-measuring charter-vs-no-charter. |
...
improved wall-clock throughput. Treat results as "does ZPR5 change reasoning
patterns in controlled conditions," consistent with the base harness's own
...
work; it is logged in `humanpending.md`.
